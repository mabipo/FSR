...
for ti=1:length(tt)
    Power_tg(j)= 10*log10(sum(Power_tg_ist));
...
% w = 1e-9; %pulse width
% d= w/2:10000*w:length(tt); %delay vector
% y=pulstran(tt,d,'rectpuls',w); 
% plot(tt,y); 
...
sigma_ground=1.5e-2; %Conductivity
 ...
 j=1
    ...
    for fo=1:size(fan_origin,1)
        ...
        Doppler_signature1=TRP_def(rx,tx,fan_pos,f_c,FSCS,epsR,sigma_ground,t_end(j),sRate);%TRP
